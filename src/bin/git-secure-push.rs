...
use git_rsl::{BranchName, RemoteName, secure_push_with_cleanup};
use clap::{App, Arg};
...
fn main() {
    ...
    if let Err(ref e) = secure_push_with_cleanup(&mut repo, &RemoteName::new(&remote), &BranchName::new(&branch)) {
        handle_error(e);
        process::exit(1);
    }
    println!("Success!")
