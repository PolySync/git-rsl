...
fn git_branch() {
    let mut context = setup_fresh();
    {
        ...
        assert!(context.local.find_branch(&random_branch_name, BranchType::Local).is_ok());
    }
...
fn git_commit() {
    let mut context = setup_fresh();
    {
        ...
        let commit = context.local.head().expect("failed to get head").peel_to_commit().expect("failed to get latest commit...");
...
fn git_push() {
    let mut context = setup_fresh();
    {
        ...
        let oid = context.local.head().expect("failed to get head").target().expect("failed to get C2 oid...");
